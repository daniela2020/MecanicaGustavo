<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <title>Mecânica Gustavo</title>
        <link rel="stylesheet" type="text/css"  href="estilo.css" />

    </head>
    <body>
        <!-- vspace - define o tamanho da margem nas bordas superior e inferior da imagem.
        hspace - define o tamanho da margem nas bordas esquerda e direita da imagem. -->
        <!-- <img src=Logo_Nova2.jpg> Imagem pequena do logo-->

        <img src="imagens/logo.png" vspace="10px" hspace="5px" border="5px" align="left"/>


           <ul class="menu">
                
                <li><a href="index.php">HOME</a></li>

                <li><a href="empresa.php">EMPRESA</a></li>
                    
                <li><a href="pecas.php">PEÇAS</a></li>
                                       
                <li><a href="contato.php">CONTATO</a></li>

                <li><a href="Administrador.php">ADMINISTRADOR</a></li>

                <li class="dropdown">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Serviços <span class="caret"></span></a>
          <ul class="dropdown-menu">
              <li><a href="Servicos.php">Motor</a></li>
              <li><a href="Servicos.php">Caixa de Câmbio</a></li>
              <li><a href="Servicos.php">Diferencial</a></li>
              <li><a href="Servicos.php">Direção Hidráulica</a></li>
              <li><a href="Servicos.php">Suspensão e Freios</a></li>
              <li><a href="Servicos.php">Reparos Mecânicos</a></li>
            
            
           
            </ul>


        <?php
        // put your code here
        ?>

    </body>
</html>
