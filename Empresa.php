<!Doctype Html>
<html>
    <head>
        <link rel="stylesheet"type="text/css" href=stylle.css>
        <meta charset=utf-8>
        <link rel="icon" href="favicon.ico">
        <title>Mecânica Gustavo</title></head>

    <body>
        <img src="imagens/logo.png" vspace="10px" hspace="0px" border="5px" align="left"/>


        <nav>
            <ul>
                <li><a href=index.php>Home</a></li>
                <li><a href=Empresa.php>Empresa</a></li>
                <li><a href=Pecas.php>Peças</a></li>
                <li><a href=Servicos.php>Serviços</a></li>
                <li><a href=Contato.php>Contato</a></li>
                <li><a href=Administrador.php>Administrador</a></li>
                <li><a href=Cadastro.php>Cadastro Cliente</a></li>
            </ul>
        </nav>

        <h1>Breve Histórico</h1>

        <section>
            <p> &nbsp &nbsp A Mecânica Gustavo, se localiza na Av. Ipiranga, 536 – Bairro Kurtz, Santo Ângelo, atua no mercado desde 27/08/1985.</p>
            <p>Atua no ramo de Peças e Prestação de Serviços para Caminhões e Camionetas da linha diesel e também na parte hidráulica de veículos da linha leve.</p><br>
            <p>&nbsp &nbsp Desde a sua criação, busca superar obstáculos, comercializando produtos diversificados de qualidade e serviços especializados, com comprometimento,</p> competitividade e qualidade, no intuito de garantir a satisfação do cliente.<br>
            <p>&nbsp &nbsp Prima por princípios fundamentais para atingir seus objetivos:<br></p>
            <h4> Honestidade<br></h4>
            <h4> Respeito<br></h4>
            <h4> Ética<br></h4>
            <h4> Competência<br></h4>
            <h4> Agilidade<br></h4>


            <hr> <h1>Política Da Empresa</h1><hr>

            <h4>Nossa política de relacionamento está pautada em aspectos essenciais:<br></h4>
            ► Excelência no atendimento, buscando satisfazer as expectativas dos clientes;<br>
            ► Transparência nas relações com o cliente;<br>
            ► Melhoria contínua da qualidade dos serviços, investindo em equipamentos e ferramentas de última geração;<br>
            ► Treinamentos e Capacitação dos Funcionários, com foco nos resultados;<br>
            ► Controle dos impactos ambientais e riscos ocupacionais.<br>
        </section>
        <?php
        echo"                 <h1>Missão</h1><br>";
        echo"<h3><center>Busca constante de aprimoramento nos serviços prestados e fortalecimento nas relações com clientes e fornecedores.</h3></center><br>";

        echo" <h1>Visão</h1><br>";

        echo"<h3><center> Objetiva manter-se na liderança do segmento de vendas de peças e prestação de serviços, proporcionando a satisfação dos clientes.</h3></center><br>";

        echo" <h1>Valores</h1>";

        echo"<h3> <center>Atuar positivamente no mercado, com ética, respeito, e confiabilidade.</h3></center><br>";
        ?>


    </body>
</html>

<h1>Colaboradores</h1> <center><img src="imagens/TurmaMecGustavo.png"></center></br>


<br><br>


<nav aria-label="Page navigation example">
    <ul class="pagination">
        <li class="page-item"><a class="page-link" href="index.php">Anterior</a></li>
        <li class="page-item"><a class="page-link" href="index.php">1</a></li>
        <li class="page-item"><a class="page-link" href="Empresa.php">2</a></li>
        <li class="page-item"><a class="page-link" href="Pecas.php">3</a></li>
        <li class="page-item"><a class="page-link" href="Servicos.php">4</a></li>
        <li class="page-item"><a class="page-link" href="Contato.php.php">5</a></li>
        <li class="page-item"><a class="page-link" href="Administrador.php">6</a></li>
        <li class="page-item"><a class="page-link" href="Cadastro.php">7</a></li>
        <li class="page-item"><a class="page-link" href="Pecas.php">Próximo</a></li>

    </ul>
</nav>

<footer>
    <address>
        Fones: 55 3312 3127 . 55 99961 2384 . 55 99905 2001 <br>
        E-Mail: mecanicagustavo@mecanicagustavo.com.br<br>
        Avenida Ipiranga, 536, Bairro Kurtz - Santo Ângelo - RS<br>
        Copyright 2018 - Mecânica Gustavo <br/>
    </address>

</footer>

</body>
</html>